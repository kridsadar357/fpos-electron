...
const SalesByNozzle = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            let url = 'http://localhost:3001/api/reports/sales-by-nozzle?';
            if (isDaily) {
    ...
    const columns = [
        ...
        { header: 'มือจ่าย (Nozzle)', accessor: 'nozzle_number', width: '15%', align: 'center' },
        { header: 'สินค้า (Product)', accessor: 'product_name', width: '25%' },
        { header: 'จำนวนรายการ (Transactions)', accessor: 'transaction_count', width: '15%', align: 'right' },
        { header: 'ปริมาณ (Liters)', accessor: 'total_liters', width: '15%', align: 'right', render: (row) => parseFloat(row.total_liters).toFixed(2) },
        { header: 'จำนวนเงิน (Amount)', accessor: 'total_amount', width: '10%', align: 'right', render: (row) => parseFloat(row.total_amount).toFixed(2) }
    ];
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <ReportViewer
                title={`รายงานยอดขายตามหัวจ่าย (${isDaily ? 'รายวัน' : 'รายกะ'})`}

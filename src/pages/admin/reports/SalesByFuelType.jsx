...
const SalesByFuelType = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            let url = 'http://localhost:3001/api/reports/sales-by-fuel-type?';
            if (isDaily) {
    ...
    const columns = [
        { header: 'สินค้า (Product)', accessor: 'product_name', width: '30%' },
        { header: 'ราคาต่อหน่วย (Price)', accessor: 'current_price', width: '15%', align: 'right', render: (row) => parseFloat(row.current_price).toFixed(2) },
        { header: 'จำนวนรายการ (Transactions)', accessor: 'transaction_count', width: '20%', align: 'right' },
        { header: 'ปริมาณ (Liters)', accessor: 'total_liters', width: '20%', align: 'right', render: (row) => parseFloat(row.total_liters).toFixed(2) },
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <ReportViewer
                title={`รายงานยอดขายตามประเภทน้ำมัน (${isDaily ? 'รายวัน' : 'รายกะ'})`}
...
export default SalesByFuelType;
